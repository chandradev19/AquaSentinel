...
const Register = () => {
  ...
  const [formData, setFormData] = useState({ name: '', email: '', password: '', role: 'CITIZEN', phone: '', villageId: '' });
  const [villages, setVillages] = useState([]);
  const [error, setError] = useState('');
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    // Fetch available villages for selection
    ...
    <div className="min-h-screen bg-brand-background flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 font-sans relative overflow-hidden">
      ...
      >
        ...
        <form className="mt-8 space-y-5" onSubmit={handleRegister}>
          ...
          <div>
            <label className="block text-xs font-semibold text-brand-secondaryText mb-1.5 uppercase tracking-wider">Phone Number</label>
            <input
              type="tel"
              className="luxury-input"
          ...
          <div>
            <label className="block text-xs font-semibold text-brand-secondaryText mb-1.5 uppercase tracking-wider">Password</label>
            <input
              type="password"
              required
              className="luxury-input"
          ...
          <div>
            <label className="block text-xs font-semibold text-brand-secondaryText mb-1.5 uppercase tracking-wider">Requested Clearance Level</label>
            <select
            ...
            >
              ...
              <option value="HEALTH_WORKER">Health Worker (Field Operations)</option>
            </select>
          </div>

          <div>
            <label className="block text-xs font-semibold text-brand-secondaryText mb-1.5 uppercase tracking-wider">
              Village / Zone Assignment <span className="text-brand-secondaryText/60 normal-case">(required for citizens)</span>
            </label>
            <select
              className="luxury-input appearance-none bg-brand-background"
            ...
            {villages.length === 0 && (
              <p className="text-xs text-brand-secondaryText/60 mt-1">Connect to backend to load villages</p>
            )}
          </div>

          <button type="submit" disabled={loading} className="w-full luxury-button mt-4 disabled:opacity-50 flex justify-center items-center">
            {loading ? (
        ...
        <div className="text-center text-sm text-brand-secondaryText border-t border-brand-border/50 pt-6 mt-6">
          Already have clearance? <Link to="/login" className="font-medium text-brand-accent hover:text-blue-400 transition-colors">Authenticate</Link>
        </div>
      </motion.div>
    </div>
  );
